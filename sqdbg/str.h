...
enum
{
	...
	kUTFEscape = 1,
	// Same as kUTFEscape, escape all backslashes as well
	kUTFEscapeQuoted,
	// Use 'u' as hex escape, replace invalid unicode with FFFD
	kUTFEscapeJSON,
};
				...
				if ( cp == '\\' && src + 1 < end )
				{
					switch ( ((unsigned char*)src)[1] )
					{
						...
						case 'x':
						{
							...
							{
								Verify( atox( string_t( src + 2, (int)sizeof(SQChar) * 2 ), &cp ) );
						...
						case 'u':
						{
							...
							{
								Verify( atox( string_t( src + 2, (int)sizeof(uint16_t) * 2 ), &cp ) );
