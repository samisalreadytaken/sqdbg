...
#define GCC_BLOCK(code) code
#else
#define GCC_BLOCK(code)
#endif
...
public:
	...
	void Start( HSQUIRRELVM vm )
	{
		...
		int idx = vm->_callsstacksize - 1 - 1; // - sqdbg_prof_start
		for ( ; idx >= 0; idx-- )
		{
		...
		for ( ; idx < vm->_callsstacksize - 1; idx++ )
		{
			const SQVM::CallInfo &ci = vm->_callsstack[idx];
...
private:
	...
	breakpoint_t *GetFunctionBreakpoint( const sqstring_t &func, const sqstring_t &funcsrc );
	...
	bool AddDataBreakpoint( const string_t &dataId, const string_t &condition, int hitsTarget, int id );
...
bool SQDebugServer::CompileDataBreakpointCondition( string_t condition, SQObjectPtr &out, unsigned int &type )
{
	...
	if ( !RunExpression( condition,
				...
				m_State == ThreadState::Suspended ? m_pCurVM->ci - 1 : m_pCurVM->ci,
				out ) )
#endif
		return false;
...
bool SQDebugServer::AddDataBreakpoint( const string_t &dataId, const string_t &strCondition, int hitsTarget, int id )
{
	...
	if ( index.ptr[0] == '1' )
	{
		...
		if ( !ref || ref->type != VARREF_OBJ ||
				!GetObj_VarRef( name, ref, obj, value ) )
		{
			m_pCurVM->_lasterror = CreateSQString( m_pCurVM, _SC("invalid object") );
			return false;
		}
		...
		{
			if ( !CompileDataBreakpointCondition( strCondition, condition, condtype ) )
				return false;
		}
...
{
	Assert( SQDebugServer::IsJumpOp( instr ) );

	int arg1 = SQDebugServer::GetJumpCount( instr );
	int sign = ( arg1 < 0 );
	if ( sign )
		arg1 = -arg1;

	for ( SQInstruction *ip = instr + SQDebugServer::GetJumpCount( instr );
			ip != instr;
			ip += sign ? 1 : -1 )
	{
		if ( ip->op == _OP_LINE )
...
#ifdef CLOSURE_ROOT
	bool ret = RunScript( vm, expression, bRoot ? _closure(ci->_closure)->_root : NULL, &m_EnvGetVal, out, multiline );
#else
		...
		const SQObject &mtenv, const SQObject &index, SQObjectPtr &value )
{
	...
	switch ( sq_type(env) )
	{
		case OT_TABLE:
		{
			if ( _table(env)->Get( index, value ) )
			{
				return true;
			}

			for ( SQTable *t = _delegable(env)->_delegate; t; t = t->_delegate )
			{
...
#endif
	{
		if ( _table(root)->Get( index, value ) )
		{
			return true;
		}

		for ( SQTable *t = _delegable(root)->_delegate; t; t = t->_delegate )
		{
			if ( t->Get( index, value ) )
			{
				return true;
...
#endif
	{
		if ( _table(root)->Set( index, value ) )
		{
			return true;
		}
...
bool SQDebugServer::Get( const objref_t &obj, SQObjectPtr &value )
{
	...
	switch ( obj.type )
	{
		...
		default: UNREACHABLE(); GCC_BLOCK( return false; );
	}
}
...
private:
	enum Token
	{
		...
		Assign,
		AssignAdd	= _assign | Add,
		AssignSub	= _assign | Sub,
		AssignMul	= _assign | Mul,
		AssignDiv	= _assign | Div,
		AssignMod	= _assign | Mod,
		AssignAnd	= _assign | BwAnd,
		AssignXor	= _assign | BwXor,
		AssignOr	= _assign | BwOr,
		AssignLS	= _assign | LShift,
		AssignRS	= _assign | RShift,
		AssignURS	= _assign | URShift,
	};
...
public:
			...
			int closer = 0 )
	{
		...
		// Using a operator stack is simpler but limits the amount
...
unary:
					...
					if ( IsValue( prevtoken ) )
					{
						while ( unaryidx != -1 )
						{
							if ( !UnaryOp( dbg, unarybuf[unaryidx] | Token::_op, val ) )
								return CompileReturnCode_Unsupported;
					...
					unaryidx++;
					unarybuf[unaryidx] = (unsigned char)( token.type & ~Token::_op );

					prevtoken = Token::Operator;
					break;
				}
				case Token::Sub:
				{
				...
				case Token::NotEq:
				{
					...
					while ( opbufidx != -1 )
					{
						int prev = opbuf[opbufidx] | Token::_op;
					...
					switch ( token.type )
					{
						case Token::LogicalAnd: if ( IsFalse( val ) ) return LexAll( closer ); break;
						case Token::LogicalOr: if ( !IsFalse( val ) ) return LexAll( closer ); break;
					}
					...
					opbufidx++;
					opbuf[opbufidx] = (unsigned char)( token.type & ~Token::_op );
					valbuf[opbufidx] = val;
				...
				case Token::Decrement:
				{
					...
					if ( prevtoken == Token::Ref )
					{
						incrop = ( token.type == Token::Increment ? 0x11 : 0x10 );
					}
					else if ( ExpectsIdentifier( prevtoken ) )
					{
						incrop = ( token.type == Token::Increment ? 0x19 : 0x18 );
					}
					else
					{
						return CompileReturnCode_Unsupported;
				...
				case '.':
				{
					if ( !IsValue( prevtoken ) || prevtoken == Token::Integer || prevtoken == Token::Float )
						return CompileReturnCode_Unsupported;
				...
				case Token::AssignLS:
				case Token::AssignRS:
				case Token::AssignURS:
				case Token::NewSlot:
				{
					if ( prevtoken != Token::Ref &&
							!( token.type == Token::NewSlot && prevtoken == Token::PendingKey ) )
						return CompileReturnCode_Unsupported;
					...
					switch ( token.type )
					{
						...
						case Token::AssignURS:
						{
							if ( !TwoArgOp( dbg, token.type & ~Token::_assign, target, val, val ) )
								return CompileReturnCode_Unsupported;
						}
						case Token::Assign:
						{
				...
				default:
				{
					...
					if ( token.type > 0 )
					{
						...
						while ( opbufidx != -1 )
						{
							if ( !TwoArgOp( dbg, opbuf[opbufidx] | Token::_op, valbuf[opbufidx], val, val ) )
								return CompileReturnCode_Unsupported;
...
private:
	...
	{
		Assert( ( op & Token::_op ) == Token::_op );

		switch ( op )
		{
			case Token::Add: return dbg->ArithOp( '+', lhs, rhs, out );
			case Token::Sub: return dbg->ArithOp( '-', lhs, rhs, out );
			case Token::Mul: return dbg->ArithOp( '*', lhs, rhs, out );
			case Token::Div: return dbg->ArithOp( '/', lhs, rhs, out );
			case Token::Mod: return dbg->ArithOp( '%', lhs, rhs, out );
			case Token::In:
			{
				switch ( sq_type(rhs) )
				{
		...
		while ( m_cur < m_end )
		{
			switch ( *m_cur )
			{
				...
				case '^':
				{
					...
					{
						m_cur++;
						token.type = Token::AssignXor;
					}
					else
					{
						token.type = Token::BwXor;
					}
				...
				case '<':
				{
					...
					else if ( m_cur < m_end && *m_cur == '=' )
					{
						...
						{
							m_cur++;
							token.type = Token::Cmp3W;
						}
						else
						{
							token.type = Token::LessEq;
						}
					}
					else if ( m_cur < m_end && *m_cur == '-' )
					{
						m_cur++;
						token.type = Token::NewSlot;
					}
					else
					{
				...
				case '>':
				{
					...
					if ( m_cur < m_end && *m_cur == '>' )
					{
						...
						if ( m_cur < m_end && *m_cur == '>' )
						{
							...
							{
								m_cur++;
								token.type = Token::AssignURS;
							}
							else
							{
								token.type = Token::URShift;
							}
						}
						else if ( m_cur < m_end && *m_cur == '=' )
						{
							m_cur++;
							token.type = Token::AssignRS;
						}
						else
						{
							token.type = Token::RShift;
						}
					}
					else if ( m_cur < m_end && *m_cur == '=' )
					{
				...
				case '&':
				{
					...
					{
						m_cur++;
						token.type = Token::AssignAnd;
					}
					else
					{
						token.type = Token::BwAnd;
					}
				...
				case '|':
				{
					...
					{
						m_cur++;
						token.type = Token::AssignOr;
					}
					else
					{
						token.type = Token::BwOr;
					}
				...
				default:
				{
					if ( _isalpha( *(unsigned char*)m_cur ) || *m_cur == '_' )
					{
								...
								token._string.len == 4 || token._string.len == 5 || token._string.len == 6 )
						{
							...
							{
								token.type = Token::Typeof;
							}
							else if ( token._string.IsEqualTo("clone") )
							{
								token.type = Token::Clone;
							}
							...
							{
								token.type = Token::Vargv;
							}
							else if ( token._string.IsEqualTo("vargc") )
							{
								token.type = Token::Vargc;
							}
#endif
						}
					}
...
xfffd:
					// Keep escaped string for invalid unicode
					m_cur += len + 2;
					break;
				}
				case 'U':
		...
		if ( *m_cur == '0' )
		{
			...
			switch ( *m_cur )
			{
				...
				default:
				{
					...
					if ( str.len > (int)FMT_OCT_LEN || !atoo( str, &token._integer ) )
					{
...
void SQDebugServer::ConvertPtr( objref_t &obj )
{
	...
	if ( obj.type & ~objref_t::PTR )
		obj.type = (objref_t::EOBJPTR)( obj.type & ~objref_t::PTR );
}
...
bool SQDebugServer::GetObj_Var( const SQObjectPtr &key, const SQObjectPtr &var, objref_t &out, SQObjectPtr &value )
{
	...
	switch ( sq_type(var) )
	{
		...
		case OT_CLASS:
		{
			...
			{
				out.type = (objref_t::EOBJPTR)( objref_t::PTR | objref_t::CLASS );
		...
		case OT_ARRAY:
		{
			...
			{
				out.type = (objref_t::EOBJPTR)( objref_t::PTR | objref_t::ARRAY );
		...
		objref_t &out, SQObjectPtr &value )
{
	...
	if ( ci && sq_type(ci->_closure) == OT_CLOSURE )
	{
		...
		if ( expression.len == 6 || expression.len == 7 )
		{
			...
			{
				// Returning a temporary vargv array here would be pointless
	...
	else
	{
		...
		{
			out.type = (objref_t::EOBJPTR)( objref_t::PTR | objref_t::READONLY );
	...
	{
		out.type = (objref_t::EOBJPTR)( objref_t::TABLE | objref_t::READONLY );
		out.src = pConstTable;
	...
	if ( ci )
	{
		...
		if ( sq_type(env) == OT_TABLE )
		{
			if ( SQTable_Get( _table(env), expression, value ) )
			{
				out.type = objref_t::TABLE;
				out.src = env;
				out.key = CreateSQString( m_pRootVM, expression );
	...
	for ( SQTable *t = root->_delegate; t; t = t->_delegate )
	{
		if ( SQTable_Get( t, expression, value ) )
		{
			out.type = objref_t::TABLE;
...
bool SQDebugServer::GetObj_VarRef( string_t &expression, const varref_t *ref, objref_t &out, SQObjectPtr &value )
{
	switch ( ref->type )
	{
		...
		{
			return GetObj_Frame( expression, ref->GetThread(), ref->scope.stack, out, value );
		}
		case VARREF_OUTERS:
		{
...
void SQDebugServer::OnRequest_StackTrace( const json_table_t &arguments, int seq )
{
	...
	if ( arguments.GetTable( "format", &format ) )
	{
		if ( !format->GetBool( "parameters", &parameters ) )
			parameters = true;
	}
...
static int _sortkeys( const SQObjectPtr *a, const SQObjectPtr *b )
{
	...
	else
	{
		...
		{
			return ( _integer(*a) - _integer(*b) ) >= 0;
		}
	}
}

#define _checkNonStringMembers(key) \
	( !sq_isstring(key) || HasEscapes( _string(key)->_val, _string(key)->_len ) )
...
void SQDebugServer::OnRequest_Variables( const json_table_t &arguments, int seq )
{
	...
	switch ( ref->type )
	{
			...
			DAP_SET_TABLE( body, 1 );
				...
				for ( int i = func->_nlocalvarinfos; i--; )
				{
					...
					if ( var._start_op <= ip + 1 && var._end_op >= ip )
					{
						...
						buf.Put('L');
						buf.Put('@');
						buf.PutInt( func->_nlocalvarinfos - i - 1 );
				...
				switch ( sq_type(target) )
				{
					...
					case OT_INSTANCE:
					{
						...
						for ( int i = 0; i < m_ClassDefinitions.size(); i++ )
						{
							...
							if ( sq_type(mm) == OT_NULL &&
									!_instance(target)->GetMetaMethod( m_pRootVM, MT_GET, mm ) )
								break;
							...
							for ( unsigned int i = 0; i < _array(def.metamembers)->_values.size(); i++ )
							{
								...
								{
									json_table_t &elem = variables.AppendTable(5);
									JSONSetString( elem, "name", member, keyflags );
									JSONSetString( elem, "value", val, flags );
									elem.SetString( "type", GetType( val ) );
									elem.SetInt( "variablesReference", ToVarRef( val ) );
									json_table_t &hint = elem.SetTable( "presentationHint", 1 );
					...
					case OT_CLOSURE:
					{
...

						{
							...
							SQInteger c = func->_ninstructions;
							for ( SQInteger i = c; i--; )
					...
					case OT_NATIVECLOSURE:
					{
								...
								_nativeclosure(target)->_nparamscheck != 0 )
						{
							...
							else
							{
								...
								if ( !( flags & kFS_Hexadecimal ) )
								{
									buf.PutInt( -_nativeclosure(target)->_nparamscheck );
								}
								else
								{
									buf.PutHex( -_nativeclosure(target)->_nparamscheck );
								}
					...
					case OT_GENERATOR:
					{
						...
						if ( _generator(target)->_state != SQGenerator::eDead )
						{
							...
							buf.Puts( source );
							buf.Put(':');
							buf.PutInt( func->GetLine( ci._ip ) );
		...
		case VARREF_INSTRUCTIONS:
		{
			...
			int ninstructions = _fp(_closure(target)->_function)->_ninstructions;
			SQFunctionProto *func = _fp(_closure(target)->_function);
...
#ifndef NATIVE_DEBUG_HOOK
						else if ( m_State == ThreadState::Suspended &&
...
void SQDebugServer::OnRequest_Next( const json_table_t &arguments, int seq )
{
	...
	if ( ( granularity.IsEqualTo( "instruction" ) && InstructionStep( vm, ci ) )
			|| Step( vm, ci ) )
	{
		m_State = ThreadState::StepOverInstruction;
	}
	else
	{
		m_State = ThreadState::StepOver;
	}
...
void SQDebugServer::OnRequest_StepIn( const json_table_t &arguments, int seq )
{
	...
	if ( ( granularity.IsEqualTo( "instruction" ) && InstructionStep( vm, ci ) )
			|| Step( vm, ci ) )
	{
		m_State = ThreadState::StepInInstruction;
	}
	else
	{
...
void SQDebugServer::OnRequest_StepOut( const json_table_t &arguments, int seq )
{
	...
	{
		m_State = ThreadState::StepOutInstruction;
	}
	else
	{
		...
		const string_t &condition, int hitsTarget, const string_t &logMessage, int id )
{
	...
	if ( GetBreakpoint( line, wsrc ) )
		return true;
#else
	if ( GetBreakpoint( line, src ) )
		return true;
#endif
	...
	breakpoint_t &bp = m_Breakpoints.append();
	memzero( &bp );
		...
		const string_t &condition, int hitsTarget, const string_t &logMessage, int id )
{
	...
	if ( GetFunctionBreakpoint( wfunc, wsrc ) )
		return true;
#else
	if ( GetFunctionBreakpoint( func, funcsrc ) )
		return true;
#endif
...
breakpoint_t *SQDebugServer::GetFunctionBreakpoint( const sqstring_t &func, const sqstring_t &funcsrc )
{
	Assert( func.ptr );
...
void SQDebugServer::DefineClass( SQClass *target, SQTable *params )
{
	...
	if ( SQTable_Get( params, _SC("name"), name ) &&
			sq_type(name) == OT_STRING && _string(name)->_len )
	{
...
#define putint( str, val ) \
	...
	putint( "stacksize     ", func->_stacksize );
	putint( "instructions  ", instrCount );
	putint( "literals      ", func->_nliterals );
	putint( "localvarinfos ", func->_nlocalvarinfos );
		...
		char *buf, int size )
{
	...
	// without setting its length as well
	char cpy[128];
	string_t expr;
	expr.Assign( cpy, min( expression.len, (int)sizeof(cpy) ) );
	memcpy( cpy, expression.ptr, expr.len );
		...
		const SQChar *sourcename, SQInteger line, const SQChar *funcname )
{
				...
				if ( pip->op == _OP_CALL )
				{
							...
							sqstring_t(_SC("suspend")).IsEqualTo( _string(_nativeclosure(val)->_name) ) )
					{
						...
						switch ( m_State )
						{
							...
							case ThreadState::StepOutInstruction:

								if ( m_pStateVM != m_pCurVM )
									break;

							case ThreadState::StepOverInstruction:
							case ThreadState::StepInInstruction:
	...
	switch ( type )
	{
		...
		case SQ_HOOK_CALL:
		{
			...
			breakpoint_t *bp = GetFunctionBreakpoint( func, src );
		...
		case SQ_HOOK_RETURN:
		{
			...
			else if ( ( m_State == ThreadState::StepOver && m_nCalls <= m_nStateCalls ) ||
...
SQInteger SQDebugServer::SQBreak( HSQUIRRELVM vm )
{
	...
	{
		if ( dbg->m_State != ThreadState::Suspended &&
...
SQInteger SQDebugServer::SQProfHook( HSQUIRRELVM vm )
{
	...
	if ( _integer(type) == SQ_HOOK_CALL || _integer(type) == SQ_HOOK_RETURN )
	{
		...
		if ( dbg && dbg->IsProfilerEnabled() )
		{
			...
			// Rare case, client disconnected while waiting for repl response
			if ( !sq_isstring(src) || !sqstring_t(_SC("sqdbg")).IsEqualTo( _string(src) ) )
			{
...
void sqdbg_get_debugger_ref( HSQUIRRELVM vm, SQObjectPtr &ref )
{
	...
	Assert( sq_isnull(ref) || sq_isuserdata(ref) );
}
...
void sqdbg_set_debugger_cached_debughook( HSQUIRRELVM vm, bool state )
{
	...
	else Assert( sq_isnull(vm->_debughook_closure) );
}

HSQDEBUGSERVER sqdbg_get_debugger_cached_debughook( HSQUIRRELVM vm )
{
	Assert( sq_isuserdata(vm->_debughook_closure) );
