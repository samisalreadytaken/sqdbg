...
void SQDebugServer::OnRequest_Completions( const json_table_t &arguments, int seq )
{
	...
	if ( ( r != CompileReturnCode_Success &&
				r != CompileReturnCode_DoesNotExist && r != CompileReturnCode_NoValue ) &&
			token.type != 0 &&
	...
	switch ( sq_type(target) )
	{
		...
		case OT_INSTANCE:
		{
					...
					_instance(target)->GetMetaMethod( vm, MT_GET, mm ) )
			{
				...
				{
					SQObjectPtr val;
					const SQObjectPtr &member = _array(def->metamembers)->_values[i];

					if ( sq_type(member) == OT_STRING && _check( _string(member) ) )
					{
						RunClosure( mm, &target, member, val );
						_set( 0, sqstring_t( _string(member) ), val );
					}
				}
			}
			...
			// methods
			{
				FOREACH_SQTABLE( base->_members, key, val )
				{
					if ( _ismethod(val) && sq_type(key) == OT_STRING && _check( _string(key) ) )
					{
						_instance(target)->Get( key, val );
			...
			else if ( sq_type(env) == OT_INSTANCE )
			{
						...
						_instance(target)->GetMetaMethod( vm, MT_GET, mm ) )
				{
					for ( unsigned int i = 0; i < _array(def->metamembers)->_values.size(); i++ )
					{
						...
						if ( sq_type(member) == OT_STRING && _check( _string(member) ) )
						{
							RunClosure( mm, &env, member, val );
							_set( 1, sqstring_t( _string(member) ), val );
						}
					}
				}
		...
		case SQ_HOOK_RETURN:
		{
					...
					vm == m_pRootVM ) // if exiting thread, step into root
			{
				Continue( vm );
					...
					m_State == ThreadState_StepOverInstruction ||
					m_State == ThreadState_StepOutInstruction ||
					// StepIn to return statement can only return 1 value,
					// ignore others if this state is kept while executing more functions
					( ( m_State == ThreadState_StepIn ||
						m_State == ThreadState_StepInInstruction ) &&
						!m_ReturnValues.size() ) )
			{
